...
ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no "$USER@$IPV6" "
    ...
    sudo kubectl --kubeconfig /etc/rancher/k3s/k3s.yaml wait --for=condition=Ready node --all --timeout=120s

    # Label the master node
    sudo kubectl --kubeconfig /etc/rancher/k3s/k3s.yaml label node $(hostname) node-role.kubernetes.io/master=
